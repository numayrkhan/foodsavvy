...
// --- Weekly helpers (ISO Monday as week anchor) ---
function startOfISOWeekUTCNoon(date = new Date()) {
  const d = new Date(
    Date.UTC(
      date.getUTCFullYear(),
      date.getUTCMonth(),
...
function ymdKeyUTC(d) {
  // returns "YYYY-MM-DD" from a Date already at UTC noon
...
// Get Weekly/Everyday Menu with categories and add-ons
app.get("/api/menus/:type", async (req, res) => {
  try {
      ...
      include: {
        items: {
          include: {
            ...
            category: true,
            menuItemAddOns: {
              // <-- Must explicitly query this table
              include: { addOn: true }, // <-- Then include related AddOn data
            },
          },
        },
      },
    ...
    // Reshape the data clearly
    const reshapedItems = menu.items.map((item) => ({
      ...item,
      addOns: item.menuItemAddOns.map((rel) => rel.addOn),
    }));

    res.json({ ...menu, items: reshapedItems });
  } catch (error) {
...
/**
 * GET /api/weekly/week?weekOf=YYYY-MM-DD
 * - weekOf is the Monday anchor (UTC noon). If omitted, uses current week.
 * - returns the configured WeeklyDay rows (active or not) for that week
 *   with the day's items → menu item + variants + addOns (flattened).
 */
app.get("/api/weekly/week", async (req, res) => {
  try {
    const { weekOf } = req.query;
    const anchor = weekOf
      ? dateKeyToUTCNoon(weekOf)
      : startOfISOWeekUTCNoon(new Date());
    ...
    const shaped = days.map((d) => ({
      ...
      date: d.date,
      active: d.active,
      // flatten addOns like your /api/menus/:type route
      items: d.items.map((wi) => {
        const mi = wi.menuItem;
        return {
          ...mi,
          addOns: (mi.menuItemAddOns || []).map((rel) => rel.addOn),
        };
      }),
    }));

    res.json({ weekOf: anchor, days: shaped });
  } catch (e) {
    console.error("weekly/week failed", e);
    res.status(500).json({ error: e.message });
  }
});

/**
 * GET /api/weekly/day/:date  (date = YYYY-MM-DD)
 * - returns a single day's listing (active or not) with items.
 */
app.get("/api/weekly/day/:date", async (req, res) => {
  try {
    const { date } = req.params;
    if (!date) return res.status(400).json({ error: "Missing date" });

    const target = dateKeyToUTCNoon(date); // store & compare at UTC noon
    const day = await prisma.weeklyDay.findFirst({
      where: { date: target },
      include: {
        items: {
          include: {
    ...
    if (!day) return res.status(404).json({ error: "No weekly menu for date" });

    const items = day.items.map((wi) => {
      const mi = wi.menuItem;
      return {
        ...mi,
        addOns: (mi.menuItemAddOns || []).map((rel) => rel.addOn),
    ...
    console.error("weekly/day failed", e);
    res.status(500).json({ error: e.message });
  }
});
