...
router.post("/menus/copy", async (req, res) => {
  ...
  const dst = await getOrCreateMenuByWeekday(toWeekday);
...
router.post("/menus/start-week", async (req, res) => {
  ...
  if (!weekStr) return res.status(400).json({ error: "weekOf (YYYY-MM-DD) required" });
  ...
  for (let d = 0; d < 7; d++) {
    ...
    let dst = await prisma.menu.findFirst({ where: { serviceDay: d, weekOf: target } });
    if (dst) {
      const ids = (await prisma.menuItem.findMany({ where: { menuId: dst.id }, select: { id: true } })).map(r => r.id);
      if (ids.length) {
        await prisma.menuItemAddOns.deleteMany({ where: { menuItemId: { in: ids } } }).catch(() => {});
        await prisma.menuVariant.deleteMany({ where: { menuItemId: { in: ids } } }).catch(() => {});
        await prisma.menuItem.deleteMany({ where: { id: { in: ids } } });
      }
    } else {
    ...
    for (const it of tmpl.items) {
      ...
      for (const v of it.variants) {
        await prisma.menuVariant.create({
          data: { label: v.label, priceCents: v.priceCents, menuItemId: newItem.id },
        });
      }
      if (Array.isArray(it.menuItemAddOns) && it.menuItemAddOns.length) {
        for (const link of it.menuItemAddOns) {
