...
router.post("/menus/item", async (req, res) => {
  try {
    ...
    const data = {
      ...
      menuId: menu.id,
      capacityPerDay:
        typeof req.body.capacityPerDay === "number"
          ? req.body.capacityPerDay
          : req.body.capacityPerDay === null
            ? null
            : undefined,
    };
    ...
    const row = id
      ...
      : await prisma.menuItem.create({
          ...
          include: {
            ...
            category: true,
            menuItemAddOns: { include: { addOn: true } },
            capacityPerDay: typeof req.body.capacityPerDay === "number"
  ? req.body.capacityPerDay
  : (req.body.capacityPerDay === null ? null : undefined),
...
router.post("/menus/copy", async (req, res) => {
  ...
  const src = await prisma.menu.findFirst({
    where: { serviceDay: fromWeekday, isTemplate: true }, // copy FROM template
    include: {
      items: {
        where: { archived: false }, // don’t clone archived
        include: { variants: true, menuItemAddOns: true },
      },
    },
  });
  if (!src)
