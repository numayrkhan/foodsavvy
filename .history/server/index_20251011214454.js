...
app.get("/api/menus/by-day", async (req, res, next) => {
  try {
    ...
    // If ?date= is provided, we still use it for capacity math (scheduled calendar day).
    // We ALSO use it to try a week-specific menu by converting to weekOf (startOfWeek).
    const dateParam = req.query.date || null;
    const targetWeekOf = dateParam ? startOfWeek(dateParam) : null;

    let menu = null;

    // 1) If a date is provided, try that exact week first
    if (targetWeekOf) {
      menu = await prisma.menu.findFirst({
        where: { serviceDay: weekday, weekOf: targetWeekOf, isActive: true },
        include: {
          items: {
    ...
    // 2) Fallback to the template for that weekday
    if (!menu) {
      menu = await prisma.menu.findFirst({
        where: { serviceDay: weekday, isTemplate: true, isActive: true },
        include: {
          items: {
            where: { archived: false },
            orderBy: { id: "asc" },
    ...
    if (req.query.date) {
      ...
      // Sum ordered quantities for that serviceDate, grouped by menuItem
    ...
    const items = (menu?.items || [])
      // only keep items that have at least one priced variant
      .filter((it) => Array.isArray(it.variants) && it.variants.some((v) => v.priceCents > 0))
      .map(({ menuItemAddOns, ...rest }) => {
        // capacity logic (null = unlimited)
        let remaining = null;
        if (remainingByItem.size && typeof rest.capacityPerDay === "number") {
    ...
    res.json({
      ...
      meta: {
        weekday,
        weekOf: menu?.weekOf ?? null,
        template: !!menu?.isTemplate,
      },
    });
  } catch (err) {
    next(err);
