...
app.get("/api/menus/by-day", async (req, res, next) => {
  try {
    ...
    // use your existing top-level prisma client
      ...
      include: {
        items: {
          ...
          include: {
            ...
            menuItemAddOns: { include: { addOn: true } }, // join table → addOn
            category: true,
          },
        },
      },
    ...
    const items = (menu?.items || [])
      // only show items that have at least one active, priced variant
      .filter(it => Array.isArray(it.variants) && it.variants.some(v => v.priceCents > 0))
      // reshape add-ons to a flat `addOns` array for the frontend
