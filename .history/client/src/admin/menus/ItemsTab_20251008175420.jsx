  ...
  const loadAll = useCallback(async (signal) => {
    ...
    try {
      const [itemsRes, catsRes, addonsRes] = await Promise.all([
        fetch(`/api/admin/menus/by-day?weekday=${weekday}`, { headers: authHeader, signal, credentials: "include" }),
      ...
      if (!itemsRes.ok || !catsRes.ok || !addonsRes.ok) {
        const [it, ct, ad] = await Promise.all([itemsRes.text(), catsRes.text(), addonsRes.text()]);
        console.error("Load failed", { items: it, categories: ct, addons: ad });
        return;
      }
      const [itemsData, catsData, addonsData] = await Promise.all([itemsRes.json(), catsRes.json(), addonsRes.json()]);
   ...
   return (
    <Box>
        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              ...
              <TableCell align="right" width={220}>Actions</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {filtered.map((i) => (
              <TableRow key={i.id} hover>
                <TableCell>
                  <Stack direction="row" spacing={1} alignItems="center">
                    {i.imageUrl ? (
                      <img src={i.imageUrl} alt="" width={36} height={36} style={{ borderRadius: 6, objectFit: "cover" }} />
                    ) : null}
                    <div>{i.name}</div>
                  </Stack>
                </TableCell>
                ...
                <TableCell>
                  {i.category?.name ? (
                    <Chip size="small" icon={<CategoryIcon fontSize="small" />} label={i.category.name} />
                  ) : "—"}
                </TableCell>
                ...
                <TableCell align="right">
                  <Stack direction="row" spacing={1} justifyContent="flex-end">
                    <Tooltip title="Variants">
                      <IconButton size="small" onClick={() => setEditing({ ...i, _panel: "variants" })}>
                        <TuneIcon fontSize="small" />
                      </IconButton>
                    </Tooltip>
                    <Tooltip title="Link add-ons">
                      <IconButton size="small" onClick={() => setEditing({ ...i, _panel: "addons" })}>
      ...
      <Dialog open={!!editing} onClose={() => setEditing(null)} maxWidth="md" fullWidth>
        <DialogTitle>
          {editing?._panel === "variants" ? "Edit Variants"
            : editing?._panel === "addons" ? "Link Add-ons"
        ...
        <DialogContent dividers>
          {editing?._panel === "variants" && (
            <VariantsEditor
              ...
              onSaved={async () => { await loadAll(); setEditing(null); }}
        ...
        {!editing?._panel && <DialogActions><Button onClick={() => setEditing(null)}>Close</Button></DialogActions>}
      </Dialog>
    </Box>
  );
}
