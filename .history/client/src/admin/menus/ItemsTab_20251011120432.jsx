...
edit scope — "templates" (default) or "week"
...
export default function ItemsTab() {
  ...
  const loadAll = useCallback(
    ...
    [authHeader, token, weekday, weekOf]
  );
  ...
  return (
    <Box>
      ...
      <Stack direction="row" spacing={2} sx={{ mb: 2 }} alignItems="center">
        ...
        {/* NEW: Week picker (YYYY-MM-DD). Empty = templates */}
        <TextField
          label="Week of (Mon)"
          type="date"
          size="small"
          ...
          sx={{ ml: 1 }}
          helperText="Leave blank to edit templates"
        />
      ...
      {/* Optional: Start Week button to clone templates → selected week (Mon–Wed) */}
      <Stack direction="row" spacing={1} sx={{ mb: 2 }}>
        <Button
          ...
          onClick={async () => {
            if (!weekOf) return;
            const weekdays = [1, 2, 3]; // Mon, Tue, Wed (owner’s pattern)
            const res = await fetch("/api/admin/menus/start-week", {
              method: "POST",
              headers: { "Content-Type": "application/json", ...authHeader },
              credentials: "include",
              body: JSON.stringify({ weekOf, weekdays }),
            });
            if (!res.ok) {
              const t = await res.text();
              return alert(`Start Week failed: ${t}`);
            }
            await loadAll();
            alert(`Cloned Mon–Wed templates into ${weekOf}.`);
          }}
        >
          Start Week (Mon–Wed)
        </Button>
