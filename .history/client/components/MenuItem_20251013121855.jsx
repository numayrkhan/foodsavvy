...
const ui = {
  group: { display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center", marginBottom: 16 },
  chip(active) {
    return {
      padding: "8px 14px",
      borderRadius: 999,
      border: `1px solid ${active ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.25)"}`,
  ...
  chipHover: { background: "rgba(255,255,255,0.14)", borderColor: "rgba(255,255,255,0.5)" },
...
export default function MenuSection({ title }) {
      ...
      try {
        const results = await Promise.all(
          [...Array(7).keys()].map(async (w) => {
            const qs = weekOfISO
              ? `/api/menus/by-day?weekday=${w}&date=${nextDayISOForWeek(weekOfISO, w)}`
              : `/api/menus/by-day?weekday=${w}&date=${nextDayISOOfThisWeek(w)}`;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const qs = weekOfISO
          ? `/api/menus/by-day?weekday=${weekday}&date=${nextDayISOForWeek(weekOfISO, weekday)}`
          : `/api/menus/by-day?weekday=${weekday}&date=${nextDayISOOfThisWeek(weekday)}`;
        const res = await fetch(qs);
        if (!res.ok) throw new Error("Failed to load menu");
        const json = await res.json();
  ...
  useEffect(() => { setActiveCategory("All"); }, [weekday, weekOfISO]);
  ...
  const scheduledISOForWeek  = weekOfISO ? nextDayISOForWeek(weekOfISO, weekday) : null;
            ...
            {visibleTabs.map((w) => {
              ...
              return (
                <button
                  ...
                  onMouseEnter={(e) => Object.assign(e.currentTarget.style, ui.chipHover)}
                  onMouseLeave={(e) => Object.assign(e.currentTarget.style, ui.chip(selected))}
                  onFocus={(e) => Object.assign(e.currentTarget.style, { ...ui.chip(selected), ...ui.chipFocus })}
                  onBlur={(e) => Object.assign(e.currentTarget.style, ui.chip(selected))}
                  onMouseDown={(e) => Object.assign(e.currentTarget.style, { ...ui.chip(selected), ...ui.chipActivePress })}
                  onMouseUp={(e) => Object.assign(e.currentTarget.style, ui.chip(selected))}
                >
                  {WEEKDAYS[w]}
                </button>
              );
            })}
            {!visibleTabs.length && <span className="text-white/70">No menus available</span>}
          </div>
        )}

        {/* Loading / error for items */}
        {loading && <p className="text-center py-8">Loading {currentTabLabel}…</p>}
              ...
              {categories.map((cat) => {
                ...
                return (
                  <button
                    ...
                    onMouseEnter={(e) => Object.assign(e.currentTarget.style, ui.chipHover)}
                    onMouseLeave={(e) => Object.assign(e.currentTarget.style, ui.chip(active))}
                    onFocus={(e) => Object.assign(e.currentTarget.style, { ...ui.chip(active), ...ui.chipFocus })}
                    onBlur={(e) => Object.assign(e.currentTarget.style, ui.chip(active))}
                    onMouseDown={(e) => Object.assign(e.currentTarget.style, { ...ui.chip(active), ...ui.chipActivePress })}
                    onMouseUp={(e) => Object.assign(e.currentTarget.style, ui.chip(active))}
                  >
                    {cat}
                  </button>
                );
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {filtered.map((item) => (
                <MenuItem
                  key={item.id}
                  item={item}
                  serviceDate={weekOfISO ? scheduledISOForWeek : scheduledISOThisWeek}
                  serviceLabel={weekOfISO ? `${WEEKDAYS[weekday]} ${new Date(serviceDate).toLocaleDateString(undefined,{month:"numeric",day:"numeric"})}` : WEEKDAYS[weekday]}
                />
              ))}
