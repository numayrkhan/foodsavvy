...
export default function MenuItem({ item, serviceDate, serviceLabel }) {
  ...
  const [imgPos, setImgPos] = useState("center bottom");  // object-position

  const handleImgLoad = (e) => {
    const { naturalWidth: w, naturalHeight: h } = e.currentTarget;
    const containerAspect = 4 / 3;        // tweak if you prefer 16/9
    const imgAspect = w / h;

    // If the image is relatively tall/narrow -> use contain (avoid cropping)
    if (imgAspect < containerAspect * 0.95) {
      setImgFit("contain");
      setImgPos("center center");
    } else {
      // Wide/normal -> cover, bias to bottom so food stays visible
      setImgFit("cover");
      setImgPos("center 80%"); // 80% pushes focus toward the bottom
    }
  };
