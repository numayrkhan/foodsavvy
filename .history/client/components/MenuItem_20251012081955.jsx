...
export default function MenuItem({ item, serviceDate, serviceLabel }) {
  ...
  const dayKey = serviceDate ? new Date(serviceDate).toISOString().slice(0, 10) : null;
  ...
  const handleAddToCart = () => {
    ...
    const mainId = `${item.id}-${selectedVariant.id}${dayKey ? `-${dayKey}` : ""}`;
  ...
  return (
    <div className="bg-gray-800 rounded-2xl shadow-md p-4 flex flex-col space-y-3">
      ...
      {/* Availability badge (uses remaining or falls back to capacityPerDay) */}
