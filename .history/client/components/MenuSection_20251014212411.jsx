...
export default function MenuSection({ title }) {
  ...
  const serviceDate = weekOfISO
    ? dateForWeekday(weekOfISO, (weekday + 6) % 7) // Sun->Mon remap
    : null;
  ...
  // next-day scheduled date for the selected tab
  ...
  const scheduledISOForWeek = weekOfISO
    ...
    // next-day scheduled date for the selected tab
const scheduledISOThisWeek = nextDayISOOfThisWeek(weekday);
const scheduledISOForWeek =
  weekOfISO ? nextDayISOForWeek(weekOfISO, weekday) : null;
