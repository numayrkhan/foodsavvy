...
export default function Cart() {
  ...
  // Build grouped + sort chronologically
  const orderedGroups = useMemo(() => {
    const groups = groupByServiceDate(cart);
    const entries = Object.entries(groups);

    const hasScheduled = entries.some(([k]) => k !== "unscheduled");
    // If there are scheduled groups, hide the unscheduled one (we want everything dated now)
    const filtered = hasScheduled
      ? entries.filter(([k]) => k !== "unscheduled")
      : entries;

    return filtered.sort(([a], [b]) => a.localeCompare(b));
  }, [cart]);

  const groupsMap = useMemo(() => Object.fromEntries(orderedGroups), [orderedGroups]);
                      ...
                      <div>
                        ...
                        {line.serviceDate && line.serviceLabel && (
                          <p className="text-xs text-white/60 mt-1">
                            Scheduled for{" "}
                            <span className="font-medium">{line.serviceLabel}</span>
                          </p>
                        )}
                      ...
                      <div className="inline-flex items-center gap-2 bg-white/10 rounded-lg">
                        ...
                        <button
                          ...
                          className={`w-8 h-8 flex items-center justify-center rounded-r-lg ${
                            atLimit ? "opacity-40 cursor-not-allowed" : "hover:bg-white/10"
                          }`}
                          aria-label={`Increase quantity of ${line.name}`}
                          disabled={atLimit}
                          title={atLimit ? "No more available for this day" : ""}
                        >
                          +
                        </button>
                      </div>
