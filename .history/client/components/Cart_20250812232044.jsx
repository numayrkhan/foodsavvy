...
function labelForDateKey(dateKey) {
  ...
  return d.toLocaleDateString(undefined, {
    ...
    month: "short",
    day: "numeric",
  }); // MDN: toLocaleDateString options
}
...
export default function Cart() {
  ...
  // Line controls
  const inc = (line) => updateQuantity(line.id, (line.quantity || 1) + 1);
  const dec = (line) => {
    ...
    <section className="container mx-auto px-4 py-10 text-white">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-4 flex items-center gap-3">
                    <div className="inline-flex items-center gap-2 bg-white/10 rounded-lg">
                      ...
                      <button
                        onClick={() => inc(line)}
                        className="w-8 h-8 flex items-center justify-center hover:bg-white/10 rounded-r-lg"
      ...
      {!!cart.length && (
        <aside className="mt-8 md:mt-10 border-t border-white/10 pt-6">
          ...
          {/* Single delivery fee is applied later (same for whole order) */}
