...
export default function MenuItem({ item, serviceDate, serviceLabel }) {
  ...
  // Removed unused scheduleDay assignment to fix compile error
  // If you need to use the next day, assign and use it where needed.
  // Example: const nextDay = idx !== -1 ? DAYS[(idx + 1) % 7] : null;
  // Currently, this logic is not used.
  const idx = tabDay ? DAYS.findIndex((d) => d.toLowerCase() === tabDay.toLowerCase()) : -1;
...
<div className="flex items-baseline justify-between gap-4">
  ...
  {remainingDisplay !== null && (
    <span
      aria-label={remainingDisplay > 0 ? `${remainingDisplay} left` : "Sold out"}
