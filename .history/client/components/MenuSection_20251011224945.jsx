...
function nextDayISOOfThisWeek(weekdayIdx) {
  ...
  // Monday anchor for THIS week
  const monday = new Date(now);
  monday.setDate(now.getDate() - ((now.getDay() + 6) % 7));
  // Base = selected weekday
  const base = new Date(monday);
  base.setDate(monday.getDate() + weekdayIdx);
  // Scheduled = next day
...
function nextDayISOForWeek(weekOfISO, w) {
  const base = new Date(dateForWeekday(weekOfISO, w)); // that week's w-day
  base.setDate(base.getDate() + 1); // next day
      ...
      try {
        const results = await Promise.all(
          [...Array(7).keys()].map(async (w) => {
            ...
            const qs = weekOf
? `/api/menus/by-day?weekday=${w}&date=${nextDayISOForWeek(startOfWeekIso(weekOf), w)}`
              : `/api/menus/by-day?weekday=${w}&date=${scheduledISO}`;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const qs = weekOf
          ? `/api/menus/by-day?weekday=${weekday}&date=${startOfWeekIso(
              weekOf
            )}`
